	...
	$.extend($.tree.plugins, {
		"contextmenu" : {
			...
			show : function(obj, t) {
				...
				$.tree.plugins.contextmenu.object.attr("class","tree-context tree-" + t.settings.ui.theme_name.toString() + "-context").css({ "left" : (tmp.left), "top" : (tmp.top + parseInt(obj.children("a:visible").height()) + 2) }).show().html(str);
			},
			hide : function () {
				...
				$.tree.plugins.contextmenu.object.hide();
			},
			exec : function (cmd) {
